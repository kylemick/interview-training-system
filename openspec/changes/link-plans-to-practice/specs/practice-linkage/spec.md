...
- **GIVEN** 存在任務ID=123,類別为'science-knowledge',時長为15分鐘,该類別題庫有足够題目
- **WHEN** 前端調用 `POST /api/plans/tasks/123/start-practice`
- **THEN** 係統应創建新的練習會話,设置`task_id=123`
- **AND** 根據任務時長自動計算題目數量(每10分鐘1題,最少1題,15分鐘=2題)
- **AND** 從題庫中随机選擇该類別的題目(數量=計算出的題目數)
- **AND** 返回會話ID、題目列表、任務信息(包含計劃名称、學生姓名、目標學校等)
- **AND** 任務狀態应更新为'in_progress'
...
- **WHEN** 用户尝試再次启動该任務
- **THEN** 係統应自動返回现有會話信息(不創建新會話)
- **AND** 返回现有會話的題目列表和進度信息
- **AND** 前端应显示"继续練習"提示,自動加载已提交的答案和反馈
- **AND** 前端应跳转到練習页面并恢复之前的進度
...
#### Scenario: Dashboard区分計劃內和自由練習統計

- **GIVEN** 用户有多个練習會話,部分關聯任務,部分为自由練習
- **WHEN** 用户访問Dashboard
- **THEN** 統計卡片应区分显示:
  - "最近練習"显示總數,并標注"計劃內: X | 自由: Y"
  - "累計題目"显示總數,并標注"計劃內: X | 自由: Y"
- **AND** 統計數據应基于會話的`task_id`字段区分
