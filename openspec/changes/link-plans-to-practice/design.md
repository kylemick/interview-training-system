...
1. **任务状态同步时机**
   - 实现: 在会话完成API中同步更新任务状态,确保原子性
   - 优化: 添加了`task_completed`字段在响应中,前端可据此判断是否显示任务完成提示

2. **题目数量处理**
   - 实现: 从任务的`category`自动选择题目,数量由后端根据任务`duration`估算
   - 注意: 如果题库题目不足,会返回错误提示,但未实现"使用现有题目"的降级方案

3. **任务信息传递**
   - 实现: 通过URL参数`taskId`传递,前端自动检测并进入任务模式
   - 优化: 任务信息横幅显示计划名称、日期、类别、进度,提升用户上下文感知

4. **完成后的引导流程**
   - 实现: 完成会话后检查剩余待办任务,显示Modal引导继续或返回
   - 体验: 全部完成时显示庆祝动画,增强成就感

### 已知限制和改进方向

1. **任务恢复功能未实现**
   - 当前: 如果用户中途退出,任务保持`in_progress`状态,但无法直接恢复
   - 改进: 需要实现"继续未完成会话"功能,检测并恢复之前的会话

2. **跳过任务功能未实现**
   - 当前: 任务只能通过完成练习来标记完成
   - 改进: 添加"跳过"按钮,允许用户直接标记完成而不练习

3. **统计数据区分**
   - 当前: Dashboard统计未区分"计划内练习"和"自由练习"
   - 改进: 分别统计两种模式的练习次数、时长、平均分等

4. **任务历史追踪**
   - 当前: 只能查看当前计划的任务,无法查看历史完成记录
   - 改进: 添加任务历史页面,支持查看过往完成的任务和统计
...
2. **任务的"跳过"功能应该如何实现?**
   - 方案A: 标记为'completed',但添加`skipped=true`标记(存储在metadata JSON字段)
   - 方案B: 新增'skipped'状态
   - **当前决策**: 方案A,简单且不改变状态枚举
   - **状态**: 待实现,已规划在tasks.md中

3. **如果用户中途放弃练习(未完成会话),任务状态应该如何?**
   - 方案A: 保持'in_progress',允许后续恢复
   - 方案B: 提供"放弃会话"按钮,回退到'pending'
   - **当前决策**: 方案A,并在下次进入时提示"继续未完成的会话"
   - **状态**: 部分实现(状态保持),恢复功能待实现

4. **是否需要限制每个任务只能创建一个会话?**
   - 方案A: 允许多次创建,但只有最新的会话关联到任务
   - 方案B: 严格限制,必须完成或放弃当前会话才能创建新会话
   - **当前决策**: 方案B,避免数据混乱,提供明确的用户引导
   - **状态**: 已实现,创建新会话前检查是否有未完成的会话

5. **任务恢复功能的实现方式?**
   - 方案A: 在任务卡片显示"继续练习"按钮,直接恢复之前的会话
   - 方案B: 在练习页面检测到未完成会话时,自动提示恢复
   - **推荐**: 方案A+B结合,提供多种恢复入口
   - **状态**: 待实现
