...
- Assess: reading comprehension, vocabulary, critical thinking, opinion analysis`;
    ...
    if (topic) {
      // 如果topic包含外部搜索信息，則作為重要參考信息
      if (topic.includes('===') || topic.includes('歷史真實') || topic.includes('外部搜索') || topic.includes('歷史真實面試題目')) {
...
現在請生成 ${count} 道題目（所有內容必須使用繁體中文）：`;
  ...
  console.log(`🤖 生成題目: ${categoryName} (${difficultyName}) x ${count}`);
  ...
  try {
    ...
    // 語言验证（仅對 english-oral 和 english-reading）
    if (category === 'english-oral' || category === 'english-reading') {
      for (const q of questions) {
        ...
        if (chineseRatio > 0.3) {
          console.warn(`⚠️ 題目語言不符合预期（中文占比${(chineseRatio * 100).toFixed(1)}%）: ${q.question_text.substring(0, 50)}...`);
        }
      }
    }

    // 验证阅读理解題目的格式（中文和英文阅读理解）
    if (category === 'chinese-reading' || category === 'english-reading') {
      for (const q of questions) {
        ...
        if (!q.question_text.includes('阅读') && !q.question_text.includes('Reading') && 
          ...
          console.warn(`⚠️ 阅读理解題目可能缺少文章內容: ${q.question_text.substring(0, 50)}...`);
        }
      }
    }
    ...
    console.error('❌ AI 生成題目失敗:', error.message);
    throw new AppError(500, `AI 生成題目失敗: ${error.message}`);
  }
}
