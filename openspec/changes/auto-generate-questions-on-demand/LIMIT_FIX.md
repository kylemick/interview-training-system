...
在实现自动生成题目功能时，错误地使用了 `LIMIT ?` 参数绑定，但 MySQL2 驱动不支持在 LIMIT 和 OFFSET 子句中使用参数绑定。
...
1. **`backend/src/utils/questionHelper.ts`**
   - 修复了 3 处 `LIMIT ?` 的使用
   - 改为直接拼接数字，但确保安全性（使用 `Math.max()` 和 `Math.min()` 限制范围）
...
已在 `openspec/project.md` 的 MySQL 访问规范中明确添加了 LIMIT/OFFSET 的使用规则，要求：
- 禁止使用参数绑定
- 必须使用安全的数字拼接
- 必须限制范围防止滥用
...
在代码审查时，请检查：
- [ ] 所有 SQL 查询中的 `LIMIT` 和 `OFFSET` 是否直接拼接数字
