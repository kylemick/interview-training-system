...
- **GIVEN** 存在任务ID=123,类别为'science-knowledge',时长为15分钟,该类别题库有足够题目
- **WHEN** 前端调用 `POST /api/plans/tasks/123/start-practice`
- **THEN** 系统应创建新的练习会话,设置`task_id=123`
- **AND** 根据任务时长自动计算题目数量(每10分钟1题,最少1题,15分钟=2题)
- **AND** 从题库中随机选择该类别的题目(数量=计算出的题目数)
- **AND** 返回会话ID、题目列表、任务信息(包含计划名称、学生姓名、目标学校等)
- **AND** 任务状态应更新为'in_progress'
...
- **WHEN** 用户尝试再次启动该任务
- **THEN** 系统应自动返回现有会话信息(不创建新会话)
- **AND** 返回现有会话的题目列表和进度信息
- **AND** 前端应显示"继续练习"提示,自动加载已提交的答案和反馈
- **AND** 前端应跳转到练习页面并恢复之前的进度
...
#### Scenario: Dashboard区分计划内和自由练习统计

- **GIVEN** 用户有多个练习会话,部分关联任务,部分为自由练习
- **WHEN** 用户访问Dashboard
- **THEN** 统计卡片应区分显示:
  - "最近练习"显示总数,并标注"计划内: X | 自由: Y"
  - "累计题目"显示总数,并标注"计划内: X | 自由: Y"
- **AND** 统计数据应基于会话的`task_id`字段区分
